...
- **FreeCAD** `configure-*-freecad` runs **`scripts/configure-freecad.sh`**, which sets **`FREECAD_USE_EXTERNAL_SMESH=ON`**, **`SMESH_DIR`** to `install/<mode>/cmake` (where **`SMESHConfig.cmake`** from the SMESH build lives), and **`CMAKE_PREFIX_PATH`** to `install/<mode>` first, then **`$CONDA_PREFIX`**, so the stack-built SMESH (and other libs under that prefix) are preferred over bundled SMESH.
